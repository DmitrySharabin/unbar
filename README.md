...
- **Auto mode** — key `0` detects the current monitor's available area and opens the window at exactly that size. Always listed first in the popup.
- **Size presets** — keys `1`–`9` select a resolution. Four sensible defaults out of the box (1280×800, 1920×1080, 1024×768, 1440×900).
- **Fully editable** — every preset slot can be edited in place: dimensions and label. Up to nine. The default (★) is whichever preset you used last (including Auto).
